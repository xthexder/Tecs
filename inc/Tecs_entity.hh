...
namespace Tecs {
    ...
    public:
        ...
        inline bool Exists(LockType &lock) const {
            auto &metadataList = lock.readAliasesWriteStorage[0] ? lock.instance.metadata.writeComponents
        ...
        inline bool Exists(EntityLock<ECSType, Permissions...> &lock) const {
            if (*this == lock.entity) {
                return Exists(lock.lock);
            } else {
        ...
        template<typename ECSType, typename... Permissions>
        inline bool Existed(EntityLock<ECSType, Permissions...> &lock) const {
            return Existed(lock.lock);
        }
        ...
        template<typename... Tn, typename ECSType, typename... Permissions>
        inline bool Has(EntityLock<ECSType, Permissions...> &lock) const {
            static_assert(!contains_global_components<Tn...>(), "Entities cannot have global components");
            if (*this == lock.entity) {
                return Has<Tn...>(lock.lock);
            } else {
                return Had<Tn...>(lock.lock);
            }
        }
        ...
        template<typename... Tn, typename ECSType, typename... Permissions>
        inline bool Had(EntityLock<ECSType, Permissions...> &lock) const {
            static_assert(!contains_global_components<Tn...>(), "Entities cannot have global components");
            return Had<Tn...>(lock.lock);
        }
        ...
        inline ReturnType &Get(LockType &lock) const {
            ...
            if (!lock.instance.template BitsetHas<CompType>(metadata)) {
                if (is_add_remove_allowed<LockType>() && !std::is_const<ReturnType>()) {
                    ...
                    throw std::runtime_error("Entity does not have a component of type: "s + typeid(CompType).name());
                }
            }

            if (lock.instance.template BitsetHas<CompType>(lock.readAliasesWriteStorage)) {
        ...
        inline T &Set(LockType &lock, T &value) const {
            ...
            if (!lock.instance.template BitsetHas<T>(metadata)) {
                if (is_add_remove_allowed<LockType>()) {
                    ...
                    throw std::runtime_error("Entity does not have a component of type: "s + typeid(T).name());
                }
            }
